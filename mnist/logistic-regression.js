...
class LogisticRegression {
  constructor(features, labels, options) {
    ...
    this.options = Object.assign({
      learningRate: 0.1,
      iterations: 100,
      dicisionBoundary: 0.5
    }, options)

    this.weights = tf.zeros([this.features.shape[1], this.labels.shape[1]]) // m, b => 2 x 1 . 0
  }
  ...
  standardize(features) {
    ...
    const filter = variance.cast('bool').logicalNot().cast('float32')

    this.mean = mean
    this.variance = variance.add(filter)
